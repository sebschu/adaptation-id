...
setwd("~/Dropbox/Uni/RA/adaptation/adaptation/models/")
...
d = convertData("./1_threshold_modals/runs/threshold-model-expected/run3_output.json")
d = reparam(d)
c3 = tidy_sampleList(d, 4, 3, list())

d = convertData("./1_threshold_modals/runs/threshold-model-expected/run4_output.json")
d = reparam(d)
...
c1 %>% ggplot(aes(x=Iteration, y=value, col=Parameter)) + geom_line() + theme(legend.position = "none")
...
write_adaptation_mle("./2_adaptation_model/bayesian-runs-balanced/", "mu-0.175_nu-3.5_cost-0.7")

write_adaptation_mle("./2_adaptation_model/bayesian-runs-balanced/", "mu-0.175_nu-4.5_cost-0.7")
...
write_adaptation_mle("./2_adaptation_model/bayesian-runs-balanced/", "prior")
