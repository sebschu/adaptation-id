...
class ParticipantModel(ThresholdModel):
  def __init__(self, config, output_path, run, subjectid):
    super().__init__(config, output_path, run, subjectid)
  ...
  def load_data(self):
    ...
    for d in raw_data["obs"]:
      if d["pair"] not in self.config["conditions"]:
        continue
...
def main():
  ...
  model = ParticipantModel(config, out_dir, args.run, subjectid=args.subject)
  model.run_mcmc()
