...
function make_slides(f) {
	...
	slides.pre_trial = slide({
		...
		present_handle: function (stim) {
			...
			$(".err").hide();
			$("#pre_trial-content").show();
			this.stim = stim;
			//$(".display_condition").html(stim.prompt);

			$("#")
			$("#pre_sent_1").text(stim["sentences"][0][1]);
			$("#pre_sent_2").text(stim["sentences"][1][1]);

			$("#pre_scene-image").attr("src", stim["image"]);



			var callback = function () {

				var total = $("#pre_slider_1").slider("option", "value") + $("#pre_slider_2").slider("option", "value") + $("#pre_slider_3").slider("option", "value");
			...
			utils.make_slider("#pre_slider_1", callback);
			utils.make_slider("#pre_slider_2", callback);
			utils.make_slider("#pre_slider_3", callback);

			$("#pre_trial").fadeIn(700);
		},
		button: function () {
			var total = $("#pre_slider_1").slider("option", "value") + $("#pre_slider_2").slider("option", "value") + $("#pre_slider_3").slider("option", "value");
			...
			if (total < .99) {
				...
				this.log_responses();
				var t = this;
				$("#pre_trial").fadeOut(300, function () {
					window.setTimeout(function () {
						_stream.apply(t);
					}, 700);
				});
		...
		log_responses: function () {
			...
			exp.data_trials.push({
				...
				"modal1": sent1[0],
				"modal2": sent2[0],
				"rating1": this.stim.reverse_sent_order == 1 ? $("#pre_slider_2").slider("option", "value") : $("#pre_slider_1").slider("option", "value"),
				"rating2": this.stim.reverse_sent_order == 1 ? $("#pre_slider_1").slider("option", "value") : $("#pre_slider_2").slider("option", "value"),
				"rating_other": $("#pre_slider_3").slider("option", "value"),
				"percentage_blue": this.stim.percentage_blue,
				"color": this.stim.color,
				"post_exposure": 0,
				"speaker": "cartoon",
	...
	slides.thanks = slide({
		...
		start: function () {
			...
			proliferate.submit(exp.data, function (data) {
				var completionURL = data["completion_URL"];
				var completionHTML = 'Thank you for taking part in the first task!<br><br>' +
					'If you are not automatically redirected, to the second task, please click on the following URL:' +
...
function init() {
	...
	exp.condition = TEST_CONDITION_ORDER + "_" + FIRST_CONDITION + "_" + SECOND_CONDITION + "_confident" + SPEAKERS["confident"],
		exp.pre_trials = _.shuffle(build_pre_trials()),
	...
	exp.structure = ["i0", "video_test", "pre_instructions", "pre_trial", "instructions", "exp_trial", "test_instructions", "post_trial", 'subj_info', 'thanks'];
