...
```{r comparison_plots, echo=FALSE, fig.width=10, fig.height=5}
  ...
  d_means %>% 
    ggplot(aes(x=percentage_blue, y=mu, col=speaker_type)) + 
      ...
      geom_line() +
      geom_point(size=1) +
      guides(col=guide_legend(title="Speaker type:")) +
      theme(legend.position="bottom") + 
      facet_wrap(~modal)
...
### Adaptation plot for paper (only included participants)

```{r paper_plots, echo=FALSE}

included_participants = read.csv("../../4_id_tasks/data/A_included_participants.csv")$x
