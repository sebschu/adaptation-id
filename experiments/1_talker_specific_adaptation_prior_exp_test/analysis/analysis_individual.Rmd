---
title: "Prior-exposure-test Experiment"
...
trials = remove_quotes(format_data(read.csv("../data/1_talker_specific_adaptation_prior_exp_test-trials.csv")))
exp_trials = remove_quotes(read.csv("../data/1_talker_specific_adaptation_prior_exp_test-exp_trials.csv"))

conditions = read.csv("../data/1_talker_specific_adaptation_prior_exp_test-condition.csv")

subj_info = read.csv("../data/1_talker_specific_adaptation_prior_exp_test-subject_information.csv") %>% select(workerid, noticed_manipulation)
...
cutoff <- data.frame(yintercept=EXCLUDE_BELOW-0.05, cutoff=factor(EXCLUDE_BELOW-0.05))


ggplot(catch_trial_perf, aes(x=factor(workerid), y=catch_prop)) + geom_bar(stat = "identity") + ggtitle("Catch trial performace") + xlab ("participant") + ylab("correct responses") + geom_hline(aes(yintercept=yintercept, linetype=cutoff, color="red"), data=cutoff) + theme(legend.position="none")
...
model = lmer(formula= auc_diff ~ cond + test_order + first_speaker_type + confident_speaker + first_speaker_type * cond + test_order * cond + (1| workerid), data = auc_d)
...
d_likelihoods.cautious = read.csv("~/Dropbox/Uni/RA/adaptation/adaptation/models/4_individual_differences/results/id-01__prior_exp_post-cautious.likelihoods")
  
d_likelihoods.confident = read.csv("~/Dropbox/Uni/RA/adaptation/adaptation/models/4_individual_differences/results/id-01__prior_exp_post-confident.likelihoods")
